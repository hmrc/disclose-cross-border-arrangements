...
class IdService @Inject() (val dateHelper: DateHelper,
  ...
  def verifyArrangementId(suppliedArrangementId: String): Future[Option[Boolean]] =
    createArrangementIdFromSuppliedString(suppliedArrangementId) match {
      ...
      case Some(validArrangementId) if validArrangementId.prefix.equals("GBA") =>
        arrangementIdRepository
          .doesArrangementIdExist(validArrangementId)
          .map(
            result => Some(result)
          )
    ...
    createDisclosureIdFromSuppliedString(suppliedDisclosureId) match {
      case Some(validDisclosureId) if validDisclosureId.prefix.equals("GBD") =>
        submissionDetailsRepository
          .doesDisclosureIdMatchEnrolmentID(validDisclosureId.value, enrolmentId)
          .map(
            result => Some(result)
          )
      case _ => Future.successful(None)
  ...
  def verifyIDs(suppliedArrangementId: String, suppliedDisclosureId: String, enrolmentId: String): Future[IdVerification] = {
    for {
  ...
  def createArrangementIdFromSuppliedString(suppliedString: String): Option[ArrangementId] =
    if (suppliedString.matches(arrangementIdRegEx)) {
      Some(ArrangementId(prefix = suppliedString.substring(0, 3), dateString = suppliedString.substring(3, 11), suffix = suppliedString.substring(11)))
    } else None
  ...
  def createDisclosureIdFromSuppliedString(suppliedString: String): Option[DisclosureId] =
    if (suppliedString.matches(disclosureIdRegEx)) {
      Some(DisclosureId(prefix = suppliedString.substring(0, 3), dateString = suppliedString.substring(3, 11), suffix = suppliedString.substring(11)))
    } else None
